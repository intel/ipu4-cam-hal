...
int Ltm::handleLtm(ia_isp_bxt_hdr_yv_grid_t* hdrYvGrid, unsigned long long timestamp, long sequence)
{
    ...
    AiqResult* feedback = getAiqResult(sequence, true);
    ...
    if ((!PlatformData::isEnableLtmThread(mCameraId)) || sequence == 0) {
        ...
        AiqResult* lastAiqResult = getAiqResult(sequence, false);
        runLtm(&(feedback->mAeResults), &(lastAiqResult->mAeResults), ltmResult);
...
int Ltm::runLtm()
{
    ...
    AiqResult *feedback = getAiqResult(inputParams->sequence, true);
    AiqResult* lastAiqResult = getAiqResult(inputParams->sequence, false);
    mLtmBinParam.sequence = inputParams->sequence;

    runLtm(&(feedback->mAeResults), &(lastAiqResult->mAeResults), ltmResult,
           &(inputParams->ltmParams));
...
int Ltm::runLtm(ia_aiq_ae_results* aeResult, ia_aiq_ae_results* lastAeResult,
                ltm_result_t *ltmResult, ia_ltm_input_params *ltmParams)
{
    LOG3A("%s", __func__);
    PERF_CAMERA_ATRACE();
    ...
    tmpLtmParams->last_ae_results = lastAeResult;
