...
public:
    ...
    void getLensSubdev() { if (!mLensSubdevName.empty()) mLensSubdev = V4l2DeviceFactory::getSubDev(mCameraId, mLensSubdevName); }
    bool isLensSubdevAvailable() {
        getLensSubdev();
        return (mLensSubdev != nullptr);
    }

private:
    int mCameraId;
    V4l2SubDev* mLensSubdev;
    string mLensSubdevName;
    string mLensName;
