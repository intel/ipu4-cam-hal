...
/**
 * Put an opened sub device
 *
 * The caller is supposed to close an opened sub device.
 */
void V4l2DeviceFactory::putSubDev(int cameraId, const string& devName)
{
    AutoMutex lock(sLock);
    V4l2DeviceFactory* factory = getInstance(cameraId);

    if (factory->mDevices.find(devName) != factory->mDevices.end()) {
        V4l2SubDev* subdev = factory->mDevices[devName];
        if (subdev) {
            subdev->closeSubDev();
            delete subdev;
        }
