...
/**
 * \struct camera_buffer_extended_info_flags_t: Specify what extended buffer info is required.
 */
typedef enum {
    BUFFER_EXTENDED_INFO_FLAG_METADATA = 1<<0,
} camera_buffer_extended_info_flags_t;

/**
 * \struct camera_buffer_extended_info_t: camera buffer extended info
 *
 * This struct is used to provide additional info beyond those in camera buffer,
 * like frame metadata, raw output from sensor, etc.
 */
typedef struct {
    int flags;
    char *metadata;
} camera_buffer_extended_info_t;
...
typedef struct {
    ...
    camera_buffer_extended_info_t *extended_info;  /**< extended info */
} camera_buffer_t;
